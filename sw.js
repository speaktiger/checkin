...
const CACHE = "speaktiger-checkin-v3";
...
/* Two ways in, because a cold tab asks for the full URL with its query and
   Safari has been inconsistent about which one matches. ignoreSearch handles
   the query; the app reads its parameters from location, not from whatever the
   cache key happened to be. */
async function cachedShell(req) {
  return (await caches.match(SHELL, { ignoreSearch: true })) ||
...
self.addEventListener("fetch", (e) => {
  ...
  e.respondWith((async () => {
    try {
      ...
      /* A host that answers 500/502/503/504 is down, not telling us the page is
         gone — the cached app is better than an error page. 404 is left alone so
         a genuinely wrong path still looks wrong. */
      if (req.mode === "navigate" && res.status >= 500) {
        const hit = await cachedShell(req);
        if (hit) return hit;
      }
      return res;
    } catch (err) {
